...
- **M9 Production Infra** — `supabase/production/` contains: `docker-compose.yml` for n8n+PostgreSQL on VPS, `.env.example`, `apply_migrations.ps1` (runs 0001-0021 via Supabase `exec_sql` RPC — note: `exec_sql` RPC may 404, SQL Editor paste is the verified path), `prod_schema.sql` (combined 0001–0021 single-paste script), `backup.ps1` (pg_dump full/tenant-scoped), `OPERATIONS.md` runbook (daily/weekly/monthly checks, incident response, tenant onboarding), pgTAP suite (`supabase/tests/pgtap/run_pgtap_tests.sql` — 42 tests covering RLS isolation, double-entry balance, commissions, statements, idempotency, salaries, inventory, reminders, reports).
