...
class DoubleEntryEngine {
  ...
  static DoubleEntryResult createSaleInvoice({
    ...
    for (final line in lines) {
      ...
      if (product.supplierId != null &&
        ...
        final dueAmount = (line.lineTotal * (1 - product.commissionRate!))
            .round();
        commissionDues.add(
          CommissionDue(
            id: const Uuid().v4(),
    ...
    final journalEntry = JournalEntry.create(
      date: invoiceDate,
      memo: memo.isEmpty
          ? 'فاتورة مبيعات ${invoiceDate.toIso8601String().split('T').first}'
          : memo,
      lines: journalLines,
      sourceType: 'sale',
    );
  ...
  static DoubleEntryResult createPurchaseInvoice({
    ...
    final journalEntry = JournalEntry.create(
      date: invoiceDate,
      memo: memo.isEmpty
          ? 'فاتورة مشتريات ${isConsignment ? '(أمانة)' : ''}'
          : memo,
      lines: journalLines,
      sourceType: 'purchase',
    );
  ...
  static DoubleEntryResult recordPayment({
    ...
    final journalEntry = JournalEntry.create(
      date: date,
      memo: note.isEmpty
          ? (isSale ? 'تحصيل فاتورة مبيعات' : 'دفع فاتورة مشتريات')
          : note,
      lines: journalLines,
      sourceType: 'payment',
      sourceId: invoice.id,
  ...
  static DoubleEntryResult settleSupplier({
    ...
    for (final invoice in invoices) {
      ...
      final settleAmount = remainingToSettle < invoice.remaining
          ? remainingToSettle
          : invoice.remaining;
      if (settleAmount > 0) {
        journalLines.add(
  ...
  static DoubleEntryResult addEmployeeMovement({
    ...
    required String
    category, // 'advance', 'goods', 'bonus', 'allowance', 'other'
    required int amount,
    required DateTime date,
    required String note,
    required Map<String, Account> accounts,
    ...
    if (!journalEntry.isBalanced) {
      throw UnbalancedEntryException(
        journalEntry,
        'Employee movement journal entry is not balanced',
      );
    }
    ...
    return DoubleEntryResult(
      ...
      updatedEntities: category == 'goods' && product != null
          ? {
              'products': {
                product.id: product.qty - (amount ~/ product.purchasePrice),
              },
            }
          : null,
    );
  ...
  static DoubleEntryResult adjustInventory({
    ...
    final journalLines = <JournalLine>[];
    final inventoryAccount = _getAccount(accounts, '1030');
    final adjustmentAccount = _getAccount(
      accounts,
      delta > 0 ? '4020' : '5020',
    );
    ...
    if (!journalEntry.isBalanced) {
      throw UnbalancedEntryException(
        journalEntry,
        'Inventory adjustment journal entry is not balanced',
      );
    }
